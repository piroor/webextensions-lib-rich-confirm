 ...
 * `userStyles` (optional): A CSS text to customize the appearance of the dialog. All selectors in this CSS are automatically rewritten to match only elements placed under the dialog's own root element, so they can never affect anything outside of the dialog UI generated by `RichConfirmDialog`. (`String`)
